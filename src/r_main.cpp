        ...
        case WM_USER_SHELLICON:
            switch(LOWORD(lParam))
            {
                ...
                case WM_RBUTTONDOWN:
					...
					// Botón de actualizaciones desactivado: No hay ningun servidor por el momento que recoja actualizaciones (01/2021)
            ...
            switch(LOWORD(wParam))
            {
                ...
                case 2: // Acerca de...
					...
					MessageBox(hwnd, _T("Radio Inteligente es una utilidad creada para usarse en conjunto con SAMP ")
						...
						_T("Colaboradores: Alejandro (Bullsito), César Ramírez (Fireplaceeee) y Sopa"),
...
void analizarMensajes() {
	...
	if (strstr(mensajeCompleto, "**[id: ") != NULL || strstr(mensajeCompleto, "**[rj ") != NULL || strstr(mensajeCompleto, ", c: ") != NULL || strstr(mensajeCompleto, "[radio id: ") != NULL || strstr(mensajeCompleto, "| ch: ") != NULL)
	{
		...
		if(radioInteligente.obtenerValorAviso(A_CUSTODIA))
		{
			...
			if(strstr(mensajeCompleto, "sospechosos") != NULL || strstr(mensajeCompleto, "sospechosas") != NULL || strstr(mensajeCompleto, "sospechoso") != NULL || strstr(mensajeCompleto, "sospechosa") != NULL)
			{
			...
			if (strstr(mensajeCompleto, "10-15") != NULL || strstr(mensajeCompleto, "'15") != NULL)
			{
				reproducirSonido(archivosDeSonido[S_SOLICITUD]);
				return;
			}
	...
	if(radioInteligente.obtenerValorAviso(A_HERIDO))
	{
		...
		if(strstr(mensajeCompleto, "se han reportado disturbios en") != NULL)
		{
			reproducirSonido(archivosDeSonido[sonidoAleatorio]);
			return;
		}

		if (strstr(mensajeCompleto, "se ha reportado un accidente en") != NULL)
		{
			reproducirSonido(archivosDeSonido[sonidoAleatorio]);
			return;
		}
	...
	if (radioInteligente.obtenerValorAviso(A_CENTRALITA)) {
		...
		if (strstr(mensajeCompleto, "* rastreo de llamada:") != NULL || strstr(mensajeCompleto, "* lugar y hechos:") != NULL) {
			reproducirSonido(archivosDeSonido[S_CENTRAL]);
			return;
		}
	}
